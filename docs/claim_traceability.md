...
|----|--------|----------|--------|
| T1 | `outputs/tables/smoke_test_results.csv` matches pin in `config/expected_outputs.json` | `python scripts/validate_outputs.py` → `VALIDATION PASSED` after `python reproduce_all.py` on HEAD `cdd1d6b69337ed7804c27e4816be25e502e669f3` (2026-04-12) | **VERIFIED** |
...
| G1 | Portfolio `eaa_system/system_snapshot.json` binds `repos.shared-core.commit` to this repo HEAD | Portfolio snapshot reconciled 2026-04-12: `repos.shared-core.commit` = `cdd1d6b69337ed7804c27e4816be25e502e669f3`; matches `git rev-parse HEAD` on the built tree | **VERIFIED** |
