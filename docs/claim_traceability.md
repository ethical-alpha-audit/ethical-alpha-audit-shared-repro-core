...
|----|--------|----------|--------|
| T1 | `outputs/tables/smoke_test_results.csv` matches pin in `config/expected_outputs.json` | `python scripts/validate_outputs.py` → `VALIDATION PASSED` after `python reproduce_all.py` at `c356e9b2e5fe459c2eef59426d233c80be3411bf` (2026-04-12) | **VERIFIED** |
...
| G1 | Portfolio `eaa_system/system_snapshot.json` binds `repos.shared-core.commit` to this repo HEAD | Portfolio snapshot reconciled 2026-04-12: `repos.shared-core.commit` = `c356e9b2e5fe459c2eef59426d233c80be3411bf`; matches authoritative shared-core pin | **VERIFIED** |
