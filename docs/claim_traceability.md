...
|----|--------|----------|--------|
| T1 | `outputs/tables/smoke_test_results.csv` matches pin in `config/expected_outputs.json` | `python scripts/validate_outputs.py` → `VALIDATION PASSED` after `python reproduce_all.py` on HEAD `709c016d262d0b4b0a6515a3ffb9ec42a245a35b` (2026-04-12) | **VERIFIED** |
...
| G1 | Portfolio `eaa_system/system_snapshot.json` binds `repos.shared-core.commit` to this repo HEAD | Portfolio snapshot reconciled 2026-04-12: `repos.shared-core.commit` = `709c016d262d0b4b0a6515a3ffb9ec42a245a35b`; matches `git rev-parse HEAD` on the built tree | **VERIFIED** |
