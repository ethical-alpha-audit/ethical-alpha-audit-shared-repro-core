...
|----|--------|----------|--------|
| T1 | `outputs/tables/smoke_test_results.csv` matches pin in `config/expected_outputs.json` | `python scripts/validate_outputs.py` → `VALIDATION PASSED` after `python reproduce_all.py` on HEAD `62e6ef8df2dcc1c6e9f166744b1b46c597a18713` (2026-04-12) | **VERIFIED** |
...
| G1 | Portfolio `eaa_system/system_snapshot.json` binds `repos.shared-core.commit` to this repo HEAD | Portfolio snapshot reconciled 2026-04-12: `repos.shared-core.commit` = `62e6ef8df2dcc1c6e9f166744b1b46c597a18713`; matches `git rev-parse HEAD` on the built tree | **VERIFIED** |
